...
from apps.surveys.models import Choice, Question, Section, SurveyTemplate, SurveyVersion
...
class TestRunCreateSurvey:
    def test_creates_survey_template_and_version(self):
        with (
            patch(
                "apps.core.workflows.survey.prompt_for_model",
                return_value={"title": "Q3 Survey", "description": ""},
            ),
        ...
        assert SurveyVersion.objects.filter(
            template__title="Q3 Survey", version_number=1
        ).exists()
    ...
    def test_new_survey_status_is_always_draft(self):
        with (
            patch(
                "apps.core.workflows.survey.prompt_for_model",
                return_value={"title": "Draft Check", "description": ""},
            ),
            patch("apps.core.workflows.survey.confirm", return_value=False),
        ):
            run_create_survey()
    ...
    def test_answering_yes_chains_into_create_question(self):
        ...
        with (
            patch(
                "apps.core.workflows.survey.prompt_for_model",
                return_value={"title": "Chained", "description": ""},
            ),
...
class TestRunCreateQuestion:
    ...
    def test_creates_choice_question_and_offers_manage_choices(self, survey_version):
        ...
        assert Question.objects.filter(
            text="Pick one", question_type="single_choice"
        ).exists()
...
class TestRunManageChoices:
    ...
    def test_add_choice(self, choice_question):
        ...
        with (
            patch("apps.core.workflows.choices.choose", side_effect=["add", "done"]),
        ...
        assert Choice.objects.filter(
            question=choice_question, label="Red", value="red"
        ).exists()

    def test_edit_choice(self, choice_question):
        choice = Choice.objects.create(
            question=choice_question, label="Blue", value="blue", order=0
        )

        # choose: "edit" → pick the choice → "done"
        with (
            patch(
                "apps.core.workflows.choices.choose",
                side_effect=["edit", choice, "done"],
            ),
            patch(
                "apps.core.workflows.choices.ask",
                side_effect=["Blue Updated", "blue_updated"],
            ),
            patch("apps.core.workflows.choices.ask_int", return_value=0),
        ):
            run_manage_choices(question=choice_question)
    ...
    def test_delete_choice(self, choice_question):
        choice = Choice.objects.create(
            question=choice_question, label="Green", value="green", order=0
        )
        ...
        with (
            patch(
                "apps.core.workflows.choices.choose",
                side_effect=["delete", choice, "done"],
            ),
    ...
    def test_resolves_survey_template_from_db(self, survey_version, choice_question):
        ...
        with patch(
            "apps.core.workflows.choices.choose",
            side_effect=[survey, choice_question, "done"],
        ):
...
class TestRunManageSections:
    def test_create_section(self, survey_version):
        ...
        with (
            patch(
                "apps.core.workflows.sections.choose", side_effect=["create", "done"]
            ),
            patch(
                "apps.core.workflows.sections.prompt_for_model",
                return_value={"title": "Demographics", "description": ""},
            ),
        ...
        assert Section.objects.filter(
            version=survey_version, title="Demographics"
        ).exists()
    ...
    def test_add_question_to_section(self, survey_version):
        ...
        with patch(
            "apps.core.workflows.sections.choose",
            side_effect=["add_q", section, question, "done"],
        ):
            run_manage_sections(survey=survey)
    ...
    def test_move_question_between_sections(self, survey_version):
        ...
        with patch(
            "apps.core.workflows.sections.choose",
            side_effect=["move", sec_a, question, sec_b, "done"],
        ):
            run_manage_sections(survey=survey)
