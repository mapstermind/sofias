...
- `static/ts/survey_progress.ts` — the client half of taking a survey: it mirrors `visibility.py` to hide gated modules/questions, drives the live progress bar, debounce-posts changed fields to `autosave_survey`, and renders the **Pendientes** panel (first six visible-and-unanswered questions, `y N más` beyond that, click to scroll + ring + focus, `Ir a la siguiente ↓` to walk the form). Each refresh collects the visible cards once and splits them into answered and pending, and every one of those readouts renders that same split — they cannot disagree. Question text comes from each card's `<legend>`, never a duplicated attribute. Classes written here **are** compiled — `static/ts` is an `@source` in `static/css/main.css`.
