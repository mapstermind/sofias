...
- `UserProfile` — OneToOne with `User`; `is_activated` gates app access; FK to `Company`;
  free-text `position` (cargo) and `department` (área — used by `apps/nom035`'s
  per-área company aggregation, blank falls into a "Sin área" bucket).
...
- `importers.py` — `import_users_from_csv`: transactional, per-row report; required CSV headers `email, company_reference_code, group, auth_method` (`otp`|`password`), plus an **optional** `department` column (blank/absent is fine — rows without it still import). Generates setup access codes for `password` users.
