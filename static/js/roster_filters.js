 ...
 */
function pills(dialog) {
    return Array.from(dialog.querySelectorAll('input[type="checkbox"], input[type="radio"]'));
}
function snapshot(dialog) {
    return pills(dialog).map((input) => [input, input.checked]);
}
function restore(taken) {
    for (const [input, checked] of taken)
...
function chosenRadios(dialog) {
    const chosen = new Map();
    for (const input of pills(dialog)) {
        if (input.type === "radio" && input.checked)
            chosen.set(input.name, input.value);
    }
    return chosen;
}
function radioFrom(target) {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!(dialog instanceof HTMLDialogElement) || !open)
        return;
    let chosen = chosenRadios(dialog);
    let taken = snapshot(dialog);
    ...
    (_a = document
        .getElementById("roster-filter-close")) === null || _a === void 0 ? void 0 : _a.addEventListener("click", () => dialog.close());
