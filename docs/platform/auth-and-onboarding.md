...
- Fecha de nacimiento is entered as three dropdowns and must fall in a 15-to-99-year working-age window; the dropdown offers no year outside it. `ProfileActivationForm` is a plain form rather than a `ModelForm`, so it checks that window itself and answers the employee in Spanish; `UserProfile.clean()` applies the same bound to the admin and the shell. A date in the future sits below the minimum, so the one bound covers it too.
...
| Conditional localidad picker | `apps/accounts/tests/test_views.py` | `TestSetupProfileView::test_single_location_is_auto_assigned_and_not_rendered`, `test_posted_location_never_assigned_when_field_hidden`, `test_several_locations_require_a_choice`, `test_zero_locations_activates_with_null_location` |
