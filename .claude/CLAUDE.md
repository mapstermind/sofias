...
Automatic source detection is off (`@import "tailwindcss" source(none)`), so the `@source` lines in `main.css` are the complete list of places a class may live: `templates/`, `apps/**/forms.py` (widget `attrs`), `apps/**/templatetags/*.py`, `static/ts/`, plus `@source inline(...)` for names only assembled at runtime. **Putting a class anywhere else compiles nothing** — add the location to `main.css` instead.

**Preflight does not normalize every element.** Tailwind v4's reset covers the form controls and the common text elements but leaves `fieldset` and `legend` with their user-agent defaults — a groove border, indents, and a `min-content` inline minimum that stops a flex row shrinking. `templates/surveys/_question.html` carries `m-0 p-0 border-0 min-w-0` for exactly that reason. Before styling an uncommon element, grep `output.css` to see whether it is reset at all; a surviving UA default fails no test, the same way an uncompiled class doesn't.
...
apps/            # Django apps; each app has its own CLAUDE.md with details
  ...
  nom035/        # NOM-035 valuation engine (scores → NDR) + results-page data  → apps/nom035/CLAUDE.md
...
- **Custom user model**: `AUTH_USER_MODEL = "accounts.User"`; login is by **email**, primarily via passwordless OTP. A name is three fields — `first_name`/`paternal_last_name`/`maternal_last_name` — and **`User.last_name` does not exist**: join with `get_full_name()` instead of assembling parts, and declare `list_display`/`search_fields`/`fieldsets` in full on any `ModelAdmin` built from Django's `UserAdmin`, whose defaults name `last_name` and fail `manage.py check` otherwise. Employee demographics (`sex`, `date_of_birth`, derived `age`) live on `accounts.UserProfile`, which operators and admins deliberately do not have. See `apps/accounts`.
- **Authorization**: custom permissions are declared on the unmanaged `accounts.Role` model and bundled into four groups (Admins / Principal Exec / Secondary Exec / Employees) by `python manage.py bootstrap_groups` (run this after migrating). `apps/accounts/roles.py` holds those four names, their Spanish display labels (*Administrador*, *Ejecutivo principal*, *Ejecutivo secundario*, *Empleado*) and their URL slugs — import them from there rather than retyping a name, and show a user the label, never the stored name. Views authorize on permission codenames (e.g. `can_view_dashboard`). Tests get the groups via the `bootstrap_groups` fixture in `conftest.py`.
- **Spanish UI, English code**: **everything a user sees is Spanish** — template copy, view and form validation messages, `messages.*` calls, admin metadata — in the public app as much as the admin. `LANGUAGE_CODE = "es-mx"` and `TIME_ZONE = "America/Mexico_City"` (storage stays UTC via `USE_TZ`). Django ships layer 1 — its own chrome, validation and date formats. Everything else an operator reads is metadata we write, so **every model field carries an explicit lowercase Spanish `verbose_name`, every model a Spanish `Meta.verbose_name`/`verbose_name_plural`, and every `TextChoices` a Spanish label** — without them Django derives the label from the English attribute name and leaks it onto the screen. Strings are hardcoded; there is no `gettext`, no `.po` files and no second language. `apps/core/permissions.py` rewrites the auto-generated `auth_permission` names, which Django builds from an untranslated template. `conftest.py`'s `assert_explicit_labels` fixture fails a build that forgets one — but it walks `model._meta` and nothing else. Template copy, view messages and `messages.*` calls have **no** automated guard, so English left in a template passes the whole suite; those are caught in review or not at all. See `docs/platform/localization.md`.
...
- **Testing**: pytest + pytest-django, settings `config.settings`. Tests live in each app's `tests/` package; shared fixtures (users, companies, survey chains, groups) are in the root `conftest.py`. `addopts` use `--reuse-db -x`. **There is no JavaScript test runner and none is planned**, so a green suite says nothing about `static/ts/`: Python tests can only assert the server-rendered contract — context values, template hooks, element ids, rendered copy. Client behaviour is verified by a human in the browser, so write it as DOM-in/value-out functions that stay inspectable, and hand the reviewer a specific list of what to click.
